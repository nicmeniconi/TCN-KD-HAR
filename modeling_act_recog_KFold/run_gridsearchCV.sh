...
for d in "${depth[@]}"; do
    for model in "${modelnames[@]}"; do
        for s in "${secs[@]}"; do
            ...
            # best_model_file="${out_path}/best_${model}_${d}_${s}_${seed}_acc_*.pth"
            # if find "${out_path}" -name "best_${model}_${d}_${s}_${seed}_acc_*.pth" | grep -q .; then
            ...
            # # Debugging statement
            # echo "Looking for: ${best_model_file}"
            # echo "Found file(s): ${found_file}"
