...
def remove_insidelines_file(infile, idxstart, linestoremove, outfile):
    try:
        with open(infile, 'r') as fr:
            ...
            with open(outfile, 'w') as fw:
                for line in lines:
                    # Write only the lines that are outside the range to be removed
                    if ptr < idxstart or ptr > (idxstart + linestoremove - 1):
                        fw.write(line)
                    ptr += 1
    except Exception as e:
        print(f"Error in remove_insidelines_file: {e}")
        print(f"File: {infile}, Start: {idxstart}, Lines to Remove: {linestoremove}, Outfile: {outfile}")

def write_outfile(infile, outfile):
    try:
        with open(infile, 'r') as fr:
            lines = fr.readlines()
            with open(outfile, 'w') as fw:
        ...
        print(f"Error in write_outfile: {e}")
