...
class VIDIMU(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        elif self.ins == 'VID' and self.out == 'IMU':  # New case
...
#     with torch.no_grad():
#         if 'IMU' in [ins, out] or ins == 'VIDIMU':
#             imu_data = torch.stack([dataset[i][1 if ins == 'VIDIMU' else 0] for i in train_idx])
#             imu_min = imu_data.min(dim=2, keepdim=True)[0].min(dim=0, keepdim=False)[0]
#             imu_max = imu_data.max(dim=2, keepdim=True)[0].max(dim=0, keepdim=False)[0]
#             print(f"IMU min shape: {imu_min.shape}, IMU max shape: {imu_max.shape}")
...
#         if 'VID' in [ins, out] or ins == 'VIDIMU':
#             video_data = torch.stack([dataset[i][0] for i in train_idx])
#             video_min = video_data.min(dim=2, keepdim=True)[0].min(dim=0, keepdim=False)[0]
#             video_max = video_data.max(dim=2, keepdim=True)[0].max(dim=0, keepdim=False)[0]
#             print(f"Video min shape: {video_min.shape}, Video max shape: {video_max.shape}")
...
#         # Calculate min and max values for IMU data (39 channels)
#         if 'IMU' in [ins, out] or ins == 'VIDIMU':
#             imu_data = torch.stack([dataset[i][1 if ins == 'VIDIMU' else 0] for i in train_idx])  # IMU data extraction
#             imu_min = imu_data.min(dim=0, keepdim=True)[0].min(dim=2, keepdim=True)[0][:39, :]  # Shape: [1, 39, 1]
#             imu_max = imu_data.max(dim=0, keepdim=True)[0].max(dim=2, keepdim=True)[0][:39, :]
#             print(f"Computed IMU min shape: {imu_min.shape}, IMU max shape: {imu_max.shape}")

#         # Calculate min and max values for Video data (102 channels)
#         if 'VID' in [ins, out] or ins == 'VIDIMU':
#             video_data = torch.stack([dataset[i][0] for i in train_idx])  # Video data extraction
#             video_min = video_data.min(dim=0, keepdim=True)[0].min(dim=2, keepdim=True)[0][:102, :]  # Shape: [1, 102, 1]
#             video_max = video_data.max(dim=0, keepdim=True)[0].max(dim=2, keepdim=True)[0][:102, :]
