...
class VIDIMU(Dataset):
    def __init__(self, data_dir, time_in_seconds, activities):
    ...
    def _load_data(self):
        for imu_file, video_file in zip(self.imu_files, self.video_files):
            ...
            ang_data = getJointAngleCsvAsNP(video_data, jointDict)
            ang_data = torch.tensor(ang_data.values, dtype=torch.float32)
            ...
            ang_windows = [
                ang_data[i:i + self.video_window_size]
                for i in range(0, len(ang_data) - self.video_window_size + 1, self.video_window_size)
            ]
            video_windows = [
            ...
            self.all_ang_windows.extend(ang_windows[:num_windows])
        ...
        self.all_ang_windows = torch.stack(self.all_ang_windows).permute(0, 2, 1)
    ...
    def __getitem__(self, idx):
        ...
        ang_window = self.all_ang_windows[idx]
        activity_label = self.all_activity_labels[idx]

        return video_window, ang_window, imu_window, activity_label
...
def collate_fn(batch, video_min=None, video_max=None, ang_min=None, ang_max=None, imu_min=None, imu_max=None):
    video_batch, ang_batch, imu_batch, labels = [], [], [], []
    for sample in batch:
            video, ang, imu, label = sample
            ...
            if ang_min is not None and ang_max is not None:
                ang = (ang - ang_min) / (ang_max - ang_min + 1e-8)
            imu_batch.append(imu)
            video_batch.append(video)
            ang_batch.append(ang)
            labels.append(label)


    return torch.stack(video_batch), torch.stack(ang_batch), torch.stack(imu_batch), torch.tensor(labels)


def standardize_postoang(dpath, time_in_seconds=3.34, split=0.8, batch_size=32, activities=activities, ins='VIDIMU', out='act'):
    dataset = VIDIMU(data_dir=dpath, time_in_seconds=time_in_seconds, activities=activities)
    train_idx, test_idx = train_test_split(range(len(dataset)), test_size=1 - split, random_state=42)
    train_dataset = Subset(dataset, train_idx)
    test_dataset = Subset(dataset, test_idx)

    with torch.no_grad():
        imu_min, imu_max, ang_min, ang_max, video_min, video_max = None, None, None, None, None, None

        imu_data = torch.stack([dataset[i][2] for i in train_idx])  # IMU data extraction
        ...
        ang_data = torch.stack([dataset[i][1] for i in train_idx])  # Video data extraction
        ang_min = ang_data.amin(dim=(0, 2), keepdim=True).squeeze(0)  # Shape: [102, 1]
        ang_max = ang_data.amax(dim=(0, 2), keepdim=True).squeeze(0)  # Shape: [102, 1]
        print(f"Computed Ang min shape: {ang_min.shape}, Ang max shape: {ang_max.shape}")
    ...
    collate = partial(collate_fn, video_min=video_min, video_max=video_max, ang_min=ang_min, ang_max=ang_max, imu_min=imu_min, imu_max=imu_max)
