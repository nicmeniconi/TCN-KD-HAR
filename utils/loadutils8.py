...
def standardize_vidimu(dpath, activities, time_in_seconds=3.34, split=0.8, batch_size=32, ins='VIDIMU', out='act', seed=None):
    if seed is not None:
        set_random_seed(seed)
    dataset = VIDIMU(data_dir=dpath, time_in_seconds=time_in_seconds, activities=activities, ins=ins, out=out)
    train_idx, test_idx = train_test_split(range(len(dataset)), test_size=1 - split, random_state=seed)
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, collate_fn=collate, num_workers=0)
