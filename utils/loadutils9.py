...
class VIDIMU(Dataset):
    ...
    def _load_data(self):
        ...
        print('IMU windows shape:', self.all_imu_windows.shape)
        self.all_video_windows = torch.stack(self.all_video_windows).permute(0, 2, 1)
        print('Video windows shape:', self.all_video_windows.shape)
        self.all_activity_labels = torch.tensor(self.all_activity_labels, dtype=torch.long)
        print('Activity windows shape:', self.all_activity_labels.shape)
